...
import { sendAppointmentReminder } from "../utils/mailer.js";
    ...
    async () => {
      try {
        ...
        for (const appointment of appointments) {
          try {
            // Validate appointment data before sending reminder
            if (!appointment.email || !appointment.fullName) {
              console.warn(
                `Skipping appointment ID ${appointment._id} due to missing email or fullName`
              );
              continue;
            }
        ...
        console.error("Error in reminder job:", error.stack || error.message);
      }
    },
    {
      scheduled: true,
